...
class PaligemmaTokenizer:
    ...
    def tokenize(self, prompt: str) -> tuple[np.ndarray, np.ndarray]:
        ...
        if tokens_len < self._max_len:
            ...
            mask = [True] * tokens_len + padding # if mask is True, the token is used for attention
...
class CTPPaligemmaTokenizer:
    ...
    def tokenize(self, prompt: str) -> tuple[np.ndarray, np.ndarray]:
        cleaned_text = prompt.strip().replace("_", " ").replace("\n", " ")
        # tokenize "\n" separately as the "start of answer" token
        prefix, suffix = cleaned_text.split('suffix:')
        prefix = prefix.replace('prefix: ', '').strip()
        suffix = suffix.strip()
        ...
        return np.asarray(tokens), np.asarray(mask), np.asarray(ar_mask), np.asarray(text_loss_mask)
