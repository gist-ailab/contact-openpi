...
REPO_NAME = "pi0_baseline_abs_quat/isaaclab"  # Name of the output dataset, also used for the Hugging Face Hub
RAW_DATASET_NAMES = [
    "open_door"
]
    ...
    for raw_dataset_name in RAW_DATASET_NAMES:
        ...
        for raw_data in raw_data_list:
            ...
            for demo in tqdm(demo_datas):
                ...
                joint_states = demo_datas[demo]['obs']['joint_pos']
                ...
                for i in tqdm(range(len(actions))):
                    ...
                    if action[7] == 1: # this means the gripper is open. but we need to change this to 0.0 for training model.
                        ...
                        gripper_action = 1.0
                    new_action = np.concatenate([action[:7], [gripper_action]], axis=0)
                    # change new action to float32
                    new_action = new_action.astype(np.float32)
                    # process the state
                    obs_pos = pos_states[i]
                    obs_quat = quat_states[i]
                    joint_pos = joint_states[i]
                    
                    obs_gripper_state = joint_pos[7:9]
                    new_state = np.concatenate([obs_pos, obs_quat, obs_gripper_state], axis=0)
